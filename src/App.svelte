<script lang="ts">
  ...
  let isBackgroundReady = $state(false);
</script>
...
<main>
  ...
  {#if isBackgroundReady}
    ...
    <div class="flex h-screen p-5 overflow-hidden">
      ...
      {:else}
        <div
          class="drop-shadow-xl/50 -translate-y-15 m-auto h-2/3 min-w-full max-w-full md:min-w-2/3 md:max-w-2/3 bg-[rgba(255,255,255,0.5)] backdrop-blur p-4 z-1 rounded-md overflow-hidden"
  ...
  {:else}
    <div class="grid place-items-center h-screen" out:fade>
      <SpinningText class="text-white" children="Loading • loading • loading" />
    </div>
  {/if}
</main>
